...
O processo de autoria de apresentações multimídia proposto consistem em duas fases distintas: \emph{configuração de objetos de mídia}~(Seção~\ref{subsec:midias_configuracao}), e \emph{criação de elos}~(Seção~\ref{subsec:criacao_elos}).
...
Um \emph{objeto de mídia} é identificado por seu \emph{nome} e pelo seu localizador de recurso uniforme, do inglês \emph{Uniform Resource Locator} (URL), que aponta para o arquivo de mídia correspondente. Os tipos de mídia suportados pela ferramenta são os seguintes: imagem, vídeo e áudio.
...
Com o objetivo de fornecer uma visão geral da apresentação sendo criada, para que o usuário possa visualizar as relações entre \emph{objetos de mídia}, foi criado o \emph{marcador de visão estrutural}. Esse marcador pode ser visto na Figura \ref{fig:estrutural_marker}.
...
A ferramenta de autoria BumbAR foi implementada como um componente da ferramenta Unity 3D \cite{unity}. Através da reutilização de componentes do Unity, a ferramenta BumbAR apresenta um ambiente tanto parar a autoria, com o uso de realidade aumentada, de apresentações multimídia, quanto para tocá-las.
